...
class ORMObject extends DBObject
{
    ...
    public function belongsTo($class_name, $pk = null, $fk = null) {
        if (is_null($pk))
            $pk = 'id';
        if (is_null($fk))
            $fk = strtolower($class_name . '_id');
        if (@!is_array(ORMObject::$_data[$this->className]))
            ORMObject::$_data[$this->className] = array();
        ORMObject::$_data[$this->className][strtolower($class_name)] = array('pk' => $pk,
                    'fk' => $fk,
                    'pc' => $class_name,
                    'fc' => $this->className,
                    'setter' => 'setBelongsTo',
    ...
    protected function setBelongsTo($key, $val) {
        if (!is_subclass_of($val, 'DBObject')) {
            trigger_error("Cannot assign non-DBObject to ORMObject property $key in {$trace[0]['file']} on line {$trace[0]['line']}", E_USER_NOTICE);
            return;
        }
    ...
    public function hasOne($class_name, $pk = null, $fk = null) {
        if (is_null($pk))
            $pk = 'id';
        if (is_null($fk))
            $fk = strtolower($this->className . '_id');
        if (@!is_array(ORMObject::$_data[$this->className]))
            ORMObject::$_data[$this->className] = array();
        ORMObject::$_data[$this->className][strtolower($class_name)] = array('pk' => $pk,
                    'fk' => $fk,
    ...
    public function hasMany($class_name, $pk = null, $fk = null, $joined = false) {
        ...
        if (@!is_array(ORMObject::$_data[$this->className]))
            ORMObject::$_data[$this->className] = array();
        ORMObject::$_data[$this->className][strtolower($class_name . 's')] = array('pk' => $pk,
                    'fk' => $fk,
                    'pc' => $this->className,
                    'fc' => $class_name,
                    'getter' => 'getHasMany',
        ...
        ORMObject::$_data[$this->className][strtolower($class_name . 'Ids')] = array('pk' => $pk,
                    'fk' => $fk,
                    'pc' => $this->className,
                    'fc' => $class_name,
        ...
        ORMObject::$_data[$this->className][strtolower('num' . ucfirst($class_name) . 's')] = array('pk' => $pk,
                    'fk' => $fk,
                    'pc' => $this->className,
                    'fc' => $class_name,
        ...
        ORMObject::$_data[$this->className][strtolower('clear' . ucfirst($class_name) . 's')] = array('pk' => $pk,
                    'fk' => $fk,
                    'pc' => $this->className,
                    'fc' => $class_name,
                    'func' => 'hasManyClear',
